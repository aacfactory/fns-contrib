...
import (
	...
	"github.com/aacfactory/fns-contrib/transports/standard"
...
func instance(ca []byte, key []byte) (srv transports.Transport, err error) {
	...
	if tlsErr != nil {
		err = errors.Warning("http: create ssl failed").WithCause(tlsErr)
		return
	}
	log, logErr := Log()
	if logErr != nil {
...
func SSL() (srv *tls.Config, cli *tls.Config, err error) {
	ca, caKey, caErr := afssl.CreateCA("FNS-TEST", 365)
	if caErr != nil {
		err = errors.Warning("http: create ca failed").WithCause(caErr)
		return
	}
	srv, cli, err = afssl.SSC(ca, caKey)
	if err != nil {
		err = errors.Warning("http: create ssl failed").WithCause(err)
		return
	}
	return
}
...
func Log() (log logs.Logger, err error) {
	log, err = logs.New(logs.Name("http"), logs.Color(true), logs.WithLevel(logs.DebugLevel))
	if err != nil {
		err = errors.Warning("http: create log failed").WithCause(err)
		return
	}
	return
}
