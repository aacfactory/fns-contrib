...
func (dialect *Dialect) Query(ctx specifications.Context, spec *specifications.Specification, cond specifications.Condition, orders specifications.Orders, groupBy specifications.GroupBy, offset int, length int) (method specifications.Method, query []byte, arguments []any, columns []string, err error) {
	generic, has, getErr := dialect.generics.Get(ctx, spec)
	if getErr != nil {
	...
	buf := bytebufferpool.Get()
	defer bytebufferpool.Put(buf)
	method, arguments, columns, err = generic.Query.Render(ctx, buf, cond, orders, groupBy, offset, length)
	if err != nil {
